...
{
    cout<<"SIGTERM received, scheduling shutting down..."<<endl;
    checker.Stop();
}
...
int main(int argc, char **argv)
{
	...
	// init logger
	string path = argv[1];
	string configPath = path + "/" + "configserver.xml";
	...
	cout<<"before log init"<<logxml<<" "<<configPath<<endl;
	VSLogger::Init(logxml.c_str(), "configserver");
	cout<<"after log init"<<endl;	
	
	if (!checker.Start(configPath))
	{
		cout<<"start checker failed"<<endl;
		return -1;
	}
	cout<<"befor loop"<<endl;	
	for (;;)
	{
   		sleep(3);
	}
